...
import TranscriptPreview from '../Activity1/TranscriptPreview';
import './Activity1/Activity1.css'
      ...
      axios.get(`https://activities-alset-aef528d2fd94.herokuapp.com/activityone/byId/${id}`)
        .then((response) => {
          if (response.data !== null) {
            ...
            if (response.data.instruction !== null) {
        ...
        <FormControlLabel className="switch-label"
          control={
            <Switch checked={newChain} onChange={() => {
                if (!newChain) {
                  alert("Caution: Data associated with the next five activities in this sequence will be permanently deleted");
                }
